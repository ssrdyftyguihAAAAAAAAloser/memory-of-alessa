#ifndef MOTEL_00_H
#define MOTEL_00_H

#include "common.h"

int   func_0013D080(int, int, int, int);
void  func_0015E780(int);
void  func_00164210(void);
int   func_001643D0(void);
int   func_001646F0(void);
void  func_00164750(int);
int   func_0016C540(int*, int*);
void  func_0016CA40(int);
void  func_0016CB30(int);
void  func_0018A260(int);
void  func_0018A310(int, int, int);
void  func_00190A20(int);
void  func_0019A940(void);
int   func_0019A9B0(float);
void  func_001C2290(int, float);
void  func_001C2A60(int, float);
void  func_00316C50(int);
float shGetDT(void);

extern int D_01F6DF60_motel_00;
extern int D_01F6DFC0_motel_00;
extern int D_01F6DFE0_motel_00;
extern int D_01F6E040_motel_00;
extern int D_01F6E060_motel_00;
extern int D_01F6E0C0_motel_00;
extern int D_01F6E100_motel_00;
extern float D_01F6E108_motel_00;
extern int D_1D316AC;
extern int D_1D31684;
extern int D_1D316FC;

#endif